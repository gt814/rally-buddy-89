...
- Видит свой профиль (группы, будущие тренировки).
- Видит историю посещенных тренировок и отметки неявок.
...
- Видит участников.
- Видит историю тренировок и список записавшихся.
- Отмечает неявки и снимает ошибочные отметки.
- Смотрит отчет по неявкам за текущий месяц.
...
Функции:
- Просмотр своих групп и выход из группы через `📋 Группы`.
...
### 3.7 Время фиксации перед тренировкой (`freeze_hours`)

За `freeze_hours` часов до старта:
- Нельзя записаться.
- Нельзя отменить активную запись.
- Нельзя выйти из очереди (`waitlist`).
- Нельзя отменить тренировку администратором.
...
- Для суперадмина: `➕ Создать группу` и список всех групп.
- Для админа: список только тех групп, которыми он управляет.

Внутри управления конкретной группой:
- `📅 Ближайшие тренировки` — будущие тренировки, загрузка, отмена тренировки.
- `📚 История тренировок` — завершенные тренировки, список записавшихся.
- `📊 Отчет за месяц` — сводка неявок по участникам за текущий месяц.
- `🗓 Шаблоны расписания` — список шаблонов, добавление/удаление.
- `👥 Участники` — список участников и статус блокировки.
- `🔗 Новая инвайт-ссылка` — пересоздание `invite_code`.
- `⚙️ Настройки` — изменение лимитов/времени фиксации и подсказки для текстовых команд.
- `🗑 Удалить` — удаление группы (только суперадмин).

### 3.10 История и неявки

- Прошедшей считается тренировка, где `date + end_time <= now`.
- Участник видит в `📚 История` только тренировки, на которые он был записан (`bookings.status = active`), со статусом:
  - `✅ Был` (по умолчанию),
  - `❌ Не пришел`.
- Админ в `📚 История тренировок` открывает тренировку и видит список записавшихся (`active`).
- По умолчанию у записавшихся `attended = true`; админ явно отмечает только неявки (`attended = false`).
- Если отметка неявки ошибочна, админ может снять ее (вернуть `attended = true`).

### 3.11 Отмена тренировки администратором
...
### 3.13 Ошибки и защитные проверки

Бот обрабатывает типовые случаи:
- Неверный формат команды.
- Группа не найдена.
- Недостаточно прав.
- Запись закрыта из-за времени фиксации.
