...
Deno.test("handleBook — время фиксации блокирует запись", async () => {
  ...
  assertStringIncludes(editedMessages[0].text, "время фиксации");
});
...
Deno.test("handleConfirmCancel — время фиксации блокирует отмену", async () => {
  const nearDate = new Date();
...
Deno.test("handleCancelWaitlist — время фиксации блокирует выход из очереди", async () => {
  const nearDate = new Date();
  nearDate.setHours(nearDate.getHours() + 1);
  const dateStr = nearDate.toISOString().split("T")[0];
  const timeStr = `${String(nearDate.getHours()).padStart(2, "0")}:${String(nearDate.getMinutes()).padStart(2, "0")}:00`;
...
Deno.test("handleAdminConfirmCancelSession — время фиксации блокирует отмену тренировки", async () => {
  const nearDate = new Date();
  nearDate.setHours(nearDate.getHours() + 1);
  const dateStr = nearDate.toISOString().split("T")[0];
  const timeStr = `${String(nearDate.getHours()).padStart(2, "0")}:${String(nearDate.getMinutes()).padStart(2, "0")}:00`;

  const { deps, editedMessages, sentMessages } = createMockDeps({
    sessions: [{
      data: {
        id: "s1",
        group_id: "g1",
...
// --- 19. Inline-кнопки: выбор времени фиксации ---
...
Deno.test("handleAdminEditFreeze — показывает варианты часов", async () => {
  ...
  assertStringIncludes(editedMessages[0].text, "сейчас: за 4ч");
  const btns = editedMessages[0].reply_markup.inline_keyboard.flat();
