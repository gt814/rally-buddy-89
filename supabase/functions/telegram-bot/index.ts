...
async function showGroupSchedule(chatId: number, messageId: number, user: any, groupId: string, backCallbackData = "schedule") {
  ...
  for (const s of sessions) {
    ...
    const freeze = getFreezeContext(s.date, s.start_time, group?.freeze_hours || 4);
    ...
    } else if (myBooking && myBooking.status === "waitlist" && !isFrozen) {
...
async function handleBook(chatId: number, messageId: number, user: any, sessionId: string) {
  ...
  if (freeze.isFrozen) {
    await editMessage(chatId, messageId, `⏰ Запись закрыта — время фиксации (за ${freeze.freezeHours}ч).`, {
      inline_keyboard: [[{ text: "« Назад", callback_data: `sched_${session.group_id}` }]],
    });
    return;
  }
...
async function handleConfirmCancel(chatId: number, messageId: number, user: any, sessionId: string) {
  ...
  const freeze = getFreezeContext(session.date, session.start_time, session.groups?.freeze_hours || 4);
  if (freeze.isFrozen) {
    await editMessage(chatId, messageId, `⏰ Отмена невозможна — время фиксации (за ${freeze.freezeHours}ч).`, {
      inline_keyboard: [[{ text: "« К расписанию", callback_data: `sched_${session.group_id}` }]],
    });
    return;
  }
...
async function handleCancelWaitlist(chatId: number, messageId: number, user: any, sessionId: string) {
  ...
  const freeze = getFreezeContext(session.date, session.start_time, session.groups?.freeze_hours || 4);
  if (freeze.isFrozen) {
    await editMessage(chatId, messageId, `⏰ Выход из очереди невозможен — время фиксации (за ${freeze.freezeHours}ч).`, {
      inline_keyboard: [[{ text: "« К расписанию", callback_data: `sched_${session.group_id}` }]],
    });
    return;
  }
...
async function handleAdminSchedule(chatId: number, messageId: number, user: any, groupId: string) {
  ...
  const { data: groupMeta } = await supabase
    .from("groups")
    .select("freeze_hours, timezone")
    .eq("id", groupId)
    .single();

  let text = "📅 <b>Тренировки (админ):</b>\n\n";
  const buttons: any[][] = [];
  const groupTimezone = groupMeta?.timezone || DEFAULT_TIMEZONE;
  const freezeHours = groupMeta?.freeze_hours || 4;
...
async function handleAdminConfirmCancelSession(chatId: number, messageId: number, sessionId: string) {
  ...
  const freeze = getFreezeContext(session.date, session.start_time, session.groups?.freeze_hours || 4);
  if (freeze.isFrozen) {
    await editMessage(chatId, messageId, `⏰ Отмена тренировки невозможна — время фиксации (за ${freeze.freezeHours}ч).`, {
...
async function handleUpdate(update: any) {
  ...
  if (update.callback_query) {
    ...
    if (data === "main_menu") {
      ...
      for (let i = 0; i < buttons.length; i += 4) rows.push(buttons.slice(i, i + 4));
      rows.push([{ text: "« Назад", callback_data: `aedit_${groupId}` }]);
      await editMessage(chatId, messageId, `⏰ Выберите время фиксации (сейчас: за ${current}ч):`, { inline_keyboard: rows });
    ...
    } else if (data.startsWith("aedit_gen_")) {
      const groupId = data.replace("aedit_gen_", "");
      const { data: group } = await supabase
        .from("groups")
        .select("schedule_generation_day_of_week, schedule_generation_time")
        .eq("id", groupId)
        .single();
      ...
      await editMessage(
        chatId,
        messageId,
        `🗓 <b>Генерация расписания</b>\n\n` +
        `День запуска: <b>${DAYS_FULL_RU[currentDay] || "Понедельник"}</b>\n` +
        `Время запуска: <b>${currentTime}</b>\n\n` +
        `Выберите параметр для изменения:`,
        {
          inline_keyboard: [
            [{ text: "📅 День запуска", callback_data: `aedit_gendate_${groupId}` }],
            [{ text: "⏰ Время запуска", callback_data: `aedit_gentime_${groupId}` }],
            [{ text: "« Назад", callback_data: `aedit_${groupId}` }],
          ],
        }
      );
    } else if (data.startsWith("aedit_gendate_")) {
      ...
      await editMessage(
        ...
        `Формат: HH:MM (24ч).`,
        { inline_keyboard: [[{ text: "« Назад", callback_data: `aedit_gen_${groupId}` }]] }
      );
    } else if (data.startsWith("aset_gendate_")) {
      const rest = data.replace("aset_gendate_", "");
      const underscoreIdx = rest.indexOf("_");
      const day = parseInt(rest.substring(0, underscoreIdx), 10);
      const groupId = rest.substring(underscoreIdx + 1);
      if (Number.isNaN(day) || day < 0 || day > 6) {
        await editMessage(chatId, messageId, "❌ Некорректный день запуска.");
        return;
      }
      await supabase.from("groups").update({ schedule_generation_day_of_week: day }).eq("id", groupId);
      await editMessage(chatId, messageId, `✅ День запуска изменен: <b>${DAYS_FULL_RU[day]}</b>`, {
        inline_keyboard: [
          [{ text: "✏️ Продолжить редактирование", callback_data: `aedit_gen_${groupId}` }],
      ...
      await editMessage(
        ...
        `📝 <code>/editgroup ${shortId} name Новое название</code>\n` +
        `🌍 <code>/editgroup ${shortId} timezone Europe/Berlin</code>\n` +
        `📅 <code>/editgroup ${shortId} gendate 1</code>\n` +
        `🗓 <code>/editgroup ${shortId} gentime 03:30</code>\n` +
        `или <code>/editgroup ${shortId} timezone UTC+3</code>`,
        { inline_keyboard: [[{ text: "« Назад", callback_data: `aedit_${groupId}` }]] }
      );
      ...
      const label = isMax ? "Макс. участников" : "Время фиксации";
  ...
  if (update.message?.text?.startsWith("/editgroup ")) {
    ...
    if (parts.length < 4) {
      await sendMessage(chatId, "Формат: <code>/editgroup GROUP_ID ПОЛЕ ЗНАЧЕНИЕ</code>\n\nПоля: name, max, freeze, timezone, gendate, gentime\nПример: <code>/editgroup abc12345 gendate 1</code>");
      return;
    }
    ...
    if (!dbField) {
      await sendMessage(chatId, "❌ Неизвестное поле. Допустимые: name, max, freeze, timezone, gendate, gentime.");
      return;
    }
    ...
    if (field === "max" || field === "freeze") {
      ...
      if (parsedValue === null) {
        await sendMessage(chatId, "❌ Неверный день запуска. Используйте день недели 0..6 (0=Вс, 1=Пн, ...).");
        return;
      }
